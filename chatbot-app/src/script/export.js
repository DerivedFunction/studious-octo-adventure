    ...
    try {
      ...
      */
      ...
      if (printOptions.replaceCanvas) {
        ...
        for (const [turnId, canvases] of canvasDataMap) {
          canvases.forEach((canvas) => {
            ...
            let codeEl;
            ({ codeEl, type } = ChatGPTCode.renderCodeBlock(
              type,
              canvas,
              title
            ));
      ...
      `;
        ...
        document
          .querySelectorAll(
            "style:not(#le-styles,#export-menu-styles,#token-popup-styles,#chm-styles,#export-status-styles,#print-options-styles,#pm-styles)"
          )
          .forEach((el) => {
...
`;
  ...
  function addPrintButton() {
    ...
    const btn = document.querySelectorAll(
      'article [aria-label="Copy"]:not(.attached)'
    );
    if (btn.length > 0) {
      btn.forEach((btn) => {
